...
from openharness.permissions.yolo_classifier import YoloClassifier
...
class PermissionChecker:
    ...
    def __init__(self, settings: PermissionSettings, yolo: YoloClassifier | None = None) -> None:
        self._settings = settings
        self._yolo_classifier = yolo or YoloClassifier()
    ...
    def evaluate(
        ...
        *,
        ...
        # YOLO classifier: auto-classify command safety
        if command and self._yolo_classifier:
            yolo_result = self._yolo_classifier.classify(tool_name, command)
            if yolo_result.decision == "allow":
                return PermissionDecision(allowed=True, reason=f"YOLO: {yolo_result.reason}")
            elif yolo_result.decision == "deny":
                return PermissionDecision(allowed=False, reason=f"YOLO: {yolo_result.reason}")
