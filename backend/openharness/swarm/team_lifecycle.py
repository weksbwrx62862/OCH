...
class TeamMember:
    ...
    """显式权限模式: default / acceptEdits / bypassPermissions / plan / dontAsk"""
...
def check_path_whitelist(
    ...
    """
    for ap in team.team_allowed_paths:
        if not target_path.startswith(ap.path):
            continue

        if operation == "write" and ap.tool_name in ("Read", "read"):
            return False, f"路径 {ap.path} 为只读模式"

        if operation == "execute" and ap.tool_name not in ("Bash", "bash", "Shell", "shell"):
            return False, f"路径 {ap.path} 不允许执行操作"

        log.debug(
            "路径白名单匹配: agent=%s path=%s op=%s rule=%s",
            agent_id,
            target_path,
            operation,
            ap.path,
    ...
    for legacy_path in team.allowed_paths:
        if target_path.startswith(legacy_path):
            return True, ""

    log.warning(
        "路径 %s 不在团队白名单规则中，执行 %s 操作（默认拒绝）",
        target_path,
        operation,
    )
    return False, f"路径 {target_path} 不在团队白名单中"
...
def set_member_mode(
    ...
    for k, m in team_file.members.items():
        if m.name == member_name:
            team_file.members[k] = replace(m, mode=mode)
            break
...
def set_multiple_member_modes(
    ...
    for k, m in list(team_file.members.items()):
        ...
        if new_mode is not None and m.mode != new_mode:
            team_file.members[k] = replace(m, mode=new_mode)
            any_changed = True
...
async def set_member_active(
    ...
    for k, m in list(team_file.members.items()):
        if m.name == member_name:
            team_file.members[k] = replace(m, is_active=is_active)
            break
